...
New decisions stream continuously from the LAPI. Each decision passes through a nine-step filter pipeline before reaching the AbuseIPDB API. State is persisted to a named Docker volume so daily counts and per-IP cooldowns survive container restarts.
...
```json
{"level":"info","limit":1000,"used_today":0,"cooldown":"15m0s","precheck":false,"min_duration":"0s","log_level":"info","msg":"bouncer started"}
```
...
│       ├── bbolt.go                     # ACID bbolt persistence (quota + cooldown)
│       ├── bbolt_test.go
...
The daily quota record resets automatically at UTC midnight. Cooldown entries are pruned every 200 decisions and on graceful shutdown. Both survive container restarts via the named Docker volume.
...
{"time":1739836530,"level":"debug","ip":"192.168.1.1","filter":"private-ip","detail":"ip=192.168.1.1 is private/reserved","msg":"decision filtered"}
```
...
12:15:31 DBG decision filtered ip=192.168.1.1 filter=private-ip
```
...
| Storage format | `daily` file + `cooldown/` directory | `state.db` (bbolt) |
| Volume mount | `bouncer-state:/tmp/cs-abuseipdb` | `bouncer-state:/data` |
| New config | — | `METRICS_ADDR`, `CONFIG_FILE` |
